...
export async function exportDashboardPDF({ brandConfig, filteredData, allData, selectedMonth, features, onProgress }) {
  ...
  pdf.roundedRect(M, 38, COL, 128, 4, 4, 'F')

  pdf.setFont('helvetica', 'bold')
  pdf.setFontSize(30)
  pdf.setTextColor(255)
  pdf.text('Reporte Mensual', M + 8, 26)

  pdf.setFontSize(11)
  pdf.setTextColor(255, 255, 255)
  pdf.text('Performance Dashboard', M + 8, 34)

  pdf.setFontSize(22)
  pdf.setTextColor(...accent)
  pdf.text(brandName, M + 8, 58)

  pdf.setFont('helvetica', 'normal')
  pdf.setFontSize(14)
  pdf.setTextColor(80)
  pdf.text(label, M + 8, 70)

  let cy = 84
  pdf.setFontSize(8.5)
  pdf.setFont('helvetica', 'bold')
  pdf.setTextColor(120)
  pdf.text('RESUMEN GENERAL', M + 8, cy)
  cy += 6
  ...
  const totSeg = v(filteredData.facebook?.seguidores) + v(filteredData.instagram?.seguidores) + v(filteredData.tiktok?.seguidores)
  const totAlc = v(filteredData.facebook?.alcance)    + v(filteredData.instagram?.alcance)    + v(filteredData.tiktok?.views)
  const totInt = v(filteredData.facebook?.interacciones) + v(filteredData.instagram?.interacciones) + v(filteredData.tiktok?.interacciones)
  const campInv = (filteredData.campanas  || []).reduce((s, r) => s + v(r.inversion), 0)
  const gadsInv = (filteredData.googleAds || []).reduce((s, r) => s + v(r.inversion), 0)
  ...
  y = tbl(y,
    ...
    ['facebook', 'instagram', 'tiktok'].map(p => {
      const act  = filteredData[p] || {}
      const ant  = getH(p, mesAnt)
  ...
  for (let i = 0; i < platConfigs.length; i++) {
    ...
    y = tbl(y, ['Métrica', 'Valor Actual', 'Mes Anterior', 'Variación'], kpis, {
    ...
    if (camps.length > 0) {
      ...
      const proyP = (filteredData.proyecciones || []).filter(p => p.plataforma === pc.key)
      const bMap  = new Map()
  ...
  for (const e of allEntries) {
    pdf.setFontSize(9.5)
    pdf.setTextColor(50)
    pdf.text(e.title, M + 3, ty)
    pdf.setTextColor(...accent)
    pdf.text(`Pág. ${e.page}`, W - M - 3, ty, { align: 'right' })
    const tw = pdf.getTextWidth(e.title)
    const pw = pdf.getTextWidth(`Pág. ${e.page}`)
    pdf.setDrawColor(200)
    pdf.setLineDashPattern([0.8, 0.8])
    pdf.line(M + 3 + tw + 3, ty - 0.5, W - M - 3 - pw - 3, ty - 0.5)
    pdf.setLineDashPattern([])
    ty += 7
    if (ty > H - 14) { footer(); pdf.addPage(); pageNum++; ty = 24 }
  }
  footer()
